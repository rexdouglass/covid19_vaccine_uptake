...
dat_long <- dat %>% 
            mutate_if(is.numeric, replace_na,0) %>% #There's a bunch of NA which should be zeros
               pivot_longer(-c(fips,stcode,stabbr,stname,cntycode,cntyname))
...
#therda_original_church_of_god_total_number_of_congregations_2010
...
not_missingness <- colSums( !is.na(rhs_combined_wide) ) # / nrow(rhs_combined_wide)
table(not_missingness<1000) #there are 6k that have fewer than 500 obs
colnames(rhs_combined_wide)[not_missingness<1000] #They're almost all peurto rico related

rhs_combined_wide[,not_missingness>1000]   %>% arrow::write_parquet("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs_wide/rhs_combined_wide.parquet") #add the parquet to the pile 

#rhs_combined_wide <- arrow::read_parquet("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs_wide/rhs_combined_wide.parquet") #add the parquet to the pile 
...
options(mc.cores = 64)
options(rf.cores=64, mc.cores=64)
x_train_imputed <- impute.rfsrc(data = x_train_df, #this takes a long time now
...
saveRDS(x_train_imputed, "/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/x_train_imputed.Rds")

```

# Cluster Variables symantically here

Switched to  hand coding
...
rhs_codebook <- rhs_codebook %>% 
                mutate(description_stemmed= description %>% 
                                            str_replace_all("per capita|percapita|per 1000|percent$|perc$|perc 1,000|^percent|^perc","") %>% tolower() %>% trimws() %>% str_replace("  "," ") %>% tolower() %>%
                                            str_replace_all("[0-9]","#") %>% str_replace_all("#{1,4}","#")
                         )  %>%
...
#These are the subset actually in the x
rhs_codebook_total <- rhs_codebook %>% filter(variable_clean %in% colnames(x_all) ) #subset the codebook to just what we're using

rhs_codebook_total %>% write.csv(file="/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs_codebook_total.csv")

#head(rhs_codebook$description)
head(rhs_codebook$description_sentence)

description_sentence_unique_df <- rhs_codebook_total %>% dplyr::select(dataset, description_sentence) %>% filter(!duplicated(description_sentence))

description_sentence_unique <- description_sentence_unique_df$description_sentence  
description_sentence_unique %>% length() #12,567 #13228 #19794 #29076
...
x_train_imputed2 <- readRDS("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/x_train_imputed.Rds")

x_train_imputed2
d_kl <- x_train_imputed2[,1:100]  %>% as.matrix() %>% scale() %>% parallelDist::parDist(threads = 64, method="euclidean") #%>% t()
hc <- d_kl %>% fastcluster::hclust(method="ward.D2") #yup it's just the dist that's slow. Have to 
...
library(infotheo)
tic()
  #d_mi <- x_train_imputed2[,1:1000] %>% t() %>% as.matrix() %>% discretize() %>% mutinformation()
  d_mi <- x_train_imputed2[,1:1000]  %>% t() %>% as.matrix() %>% discretize() %>% knnmi.all(k=5)
  dim(d_mi)
toc() #1 min for 1k
...
install.packages('Informeasure')


#So we're going to go through this and cluster
#We're really only concerned with acs1 , acs5 
table(description_sentence_unique_df$dataset)

#I think the idea is to cluster but only within datasets now.

condition <- description_sentence_unique_df$dataset %in% c('acs1','acs5','census2020','census_2020_actual')

#install.packages("fastcluster")
library(fastcluster)
library(reticulate)
py$sentence_embeddings %>% dim()
m <- py$sentence_embeddings[condition,] %>% as.matrix()
rownames(m) <- description_sentence_unique[condition]
dim(m) #11691 768
...
d_kl <- x_all[,1:100] %>% t() %>% parallelDist::parDist(threads = 64, method="kullback")

hc_kl <- d_kl %>% fastcluster::hclust(method="ward.D2") #yup it's just the dist that's slow. Have to 
...
loess1 <- stats::loess(y~x, data=df, span=0.075)
...
library(stringr)
df_clustered <- data.frame(description_sentence = description_sentence_unique[condition])
dim(df_clustered)
break_points = which(df$resid_125==T)
#break_points=break_points[break_points<break_points[break_points>smallest_cluster][1]] #pick the smallest just after our smallest hand picked cluster
...
#I'm getting python warnings which fill the console/rmd and crash things. 
for(i in 1:50){
  for(v in V(g)) {
    #print(v)
...
g_dendro <- g_phylo %>% phylogram::as.dendrogram.phylo()

library(dendextend)
#https://stackoverflow.com/questions/14118033/horizontal-dendrogram-in-r-with-labels
#dendr <- dendro_data(g_dendro)
#dendr$labels <- dendr$labels %>% 
#                rename(label_old=label) %>% 
#                left_join(df_clustered %>% 
#                         dplyr::select(label=description_sentence,label_old=k131) %>%   #you have to pick this last column name by hand right now
#                         mutate(label = label %>% str_replace_all('This variable measures ','')) %>%
#                         group_by(label_old) %>% mutate(var_count=n()) %>% filter(row_number()==1) %>% distinct() 
#                         ) %>% 
#                         dplyr::select(-label_old) %>% mutate(label=paste0("(",var_count,") ", label))

#p <- ggplot() + 
#      geom_segment(data=segment(dendr), aes(x=x, y=(y+1) %>% sqrt()  , xend=xend, yend=(yend+1) %>% sqrt())) + 
#      geom_text(data=label(dendr), aes(x=x, y=y+1, label=label, hjust=0), size=3) +
#      coord_flip()  + scale_y_reverse(limits=c(3.5, -2.5)) +  #
...
#ggsave(filename="/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/docs/plots/p_variable_dendro.pdf", plot=p, width = 50, height=25, limitsize = FALSE)


#Basically we're going to be replacing everything here

leaves <-  df_clustered %>% 
                 dplyr::select(label=description_sentence,label_old=k196) %>%   #you have to pick this last column name by hand right now
                 mutate(label = label %>% str_replace_all('This variable measures ','')) %>%
                 group_by(label_old) %>% mutate(var_count=n()) %>% filter(row_number()==1) %>% distinct() %>%
                 dplyr::select(-label_old) %>% mutate(label=paste0("(",var_count,") ", label))
g_df_thinned <- g %>%
        as_data_frame() %>%
        left_join(leaves %>% dplyr::select(to=label_old, label)) %>%
        mutate(to=coalesce(label, to)) %>%
        dplyr::select(from,to)
library(aphylo)
p_load(phylogram)
g_phylo_thinned <- g_df_thinned %>% as.matrix() %>% as.phylo() 
g_dendro_thinned <- g_phylo_thinned %>% phylogram::as.dendrogram.phylo()


library(dendextend)
#https://stackoverflow.com/questions/14118033/horizontal-dendrogram-in-r-with-labels

df_clustered_thinned <- ToDataFrameTypeCol(as.Node(g_dendro_thinned)) #only 8 levels so we can totally make this a table
df_clustered_thinned$level_1 <- NULL

options(knitr.kable.NA = '') #https://stackoverflow.com/questions/53678412/make-na-values-in-table-just-blank-cells-when-using-cell-spec-in-kableextra
library(kableExtra)
df_clustered_thinned %>%
...
df_clustered_thinned %>% saveRDS('/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/df_clustered_thinned.Rds')
...
df_clustered_thinned_ht %>%
  mutate(level_5=ifelse(level_2=='1' & level_3=='1' &  level_4=='1' &  level_5=='1',"race/ancestry/language",level_5)) %>%
  mutate(level_5=ifelse(level_2=='1' & level_3=='1' &  level_4=='1' &  level_5=='2',"race/ancestry/language",level_5)) %>%
  mutate(level_5=ifelse(level_2=='1' & level_3=='1' &  level_4=='1' &  level_5=='3',"race/ancestry/language",level_5)) %>%
  #mutate(level_5=ifelse(level_2=='1' & level_3=='1' &  level_4=='1' &  level_5=='1',"race/ancestry/language",level_5)) %>%
  
  set_font_size(value=7)  %>% 
  set_all_padding(value="0px")  %>%
  set_tb_padding(value="0px")  %>% 
  set_bottom_border(row = nrow(df_clustered_thinned_ht), col = everywhere) %>% 
  set_caption("Hierarchical Clustering of Demographic Features<br>2020 Census (estimated), American Community Survey (5 Year, 1 year)") %>% set_caption_pos("topleft") %>%
...
I think the aesthetic is to never split across datasets

```{r}

rhs_codebook_total

rhs_codebook_total %>% dplyr::select(dataset) %>% count(dataset)
