...
rhs_acs_percent <- acs_raw  %>%
                   filter(!description %>% str_detect("income|dollars|minutes|rooms|hours|value|structure|facilities")) %>% #This is the costly bit
...
#I don't know where but acs1 keeps sneaking in somewhere
rhs_acs_percent  %>% filter(dataset!="acs1") %>% rex_clean() %>% arrow::write_parquet(glue::glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs/rhs_acs_percent.parquet"))


rhs_acs_ammounts <- acs_raw  %>%
                   filter(description %>% str_detect("income|dollars|minutes|rooms|hours|value|structure|facilities")) %>% #This is the costly bit
                   filter(!is.na(value)) 
dim(rhs_acs_ammounts)

rhs_acs_ammounts %>% filter(dataset!="acs1")  %>% rex_clean() %>% arrow::write_parquet(glue::glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs/rhs_acs_ammounts.parquet"))

#table(rhs_acs_ammounts$dataset)
#table(rhs_acs_percent$dataset)
...
countyhealthrankings2019 <- fread("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data/countyhealthrankings/analytic_data2019.csv") %>% clean_names()
...
rhs_countyhealthrankings <- 
  countyhealthrankings2019 %>% 
  dplyr::select(
  ...
  uninsured_children_perc_sahi2018=uninsured_children_raw_value,
  ...
  preventable_hospital_stays_black_percap2018 =preventable_hospital_stays_black,
  preventable_hospital_stays_hispanic_percap2018 =preventable_hospital_stays_hispanic,
  preventable_hospital_stays_white_percap2018 =preventable_hospital_stays_white,
  
  mammography_screening_perc2018 =mammography_screening_raw_value, #Percentage of female Medicare enrollees ages 65-74 that received an annual mammography screening.
  mammography_screening_black_perc2018=mammography_screening_black,
  mammography_screening_hispanic_perc2018=mammography_screening_hispanic,
  mammography_screening_white_perc2018=mammography_screening_white,
  
  #Percentage of annual Medicare enrollees having an annual flu vaccination
  ...
  flu_vaccinations_black_perc_medicare2018 =flu_vaccinations_black,
  flu_vaccinations_hispanic_perc_medicare2018 =flu_vaccinations_hispanic,
  flu_vaccinations_white_perc_medicare2018 =flu_vaccinations_white,
  
  teen_births_percap=teen_births_raw_value, #Births per 1,000 females ages 15-19
  teen_births_black_percap=teen_births_black,
  teen_births_hispanic_percap=teen_births_hispanic,
  teen_births_white_percap=teen_births_white,
  
  infant_mortality_percap=infant_mortality_raw_value,
  infant_mortality_black_percap=infant_mortality_black,
  infant_mortality_hispanic_percap=infant_mortality_hispanic,
  infant_mortality_white_percap=infant_mortality_white,
  
  child_mortality_percap=child_mortality_raw_value,
  child_mortality_black_percap=child_mortality_black,
  child_mortality_hispanic_percap=child_mortality_hispanic,
  child_mortality_white_percap=child_mortality_white  ,

  children_in_poverty_percap=children_in_poverty_raw_value,
  children_in_poverty_black_percap=children_in_poverty_black,
  children_in_poverty_hispanic_percap=children_in_poverty_hispanic,
  children_in_poverty_white_percap=children_in_poverty_white
...
#Had to do a lot of this by hand
prri_2020 <-
  fread("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_in/prri/view-source_https___datawrapper.dwcdn.net_eK6to_semicolonseperated3_cleaned.csv", sep=";") %>%
  janitor::clean_names() %>%
  mutate(unaffiliated = unaffiliated %>% str_replace_all('[^0-9]','') %>% as.numeric() )
dim(prri_2020)

rhs_prri  <- prri_2020 %>%  
                   dplyr::select(-name,-population) %>%
                   pivot_longer(-fips_code) %>%
                    mutate(dataset='prri') %>%
                    mutate(description= paste0("percent religion ", name, " in 2020")  ) %>%
                    mutate(variable=paste0("prri_percent_religion_",name %>% tolower())) %>% 
                    filter(!is.na(value)) %>% rename(fips=fips_code)

rhs_prri %>% mutate(fips_variable=paste0(fips,variable)) %>% filter(duplicated(fips_variable)) #no dupes here yet so it's getting introduced lower

rhs_prri  %>% rex_clean() %>% arrow::write_parquet(glue::glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs/rhs_prri.parquet"))
...
rhs_long_notcounts <- rhs_long %>% dplyr::select(fips, variable, description, dataset, value) %>% 
                     filter(variable %rlike% "perc|percap|percapita*|per capita|percent" |
                            description %rlike% "percap|percapita|per capita|percent") #%>% collect() 
rhs_long_notcounts %>% spark_write_parquet("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs_long_notcounts/", mode="overwrite") #
rhs_long_notcounts <- spark_read_parquet(sc, path="/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs_long_notcounts/" ,memory=F)

rhs_long_counts <- rhs_long %>% dplyr::select(fips, variable, description, dataset, value) %>% 
                   filter(!variable %rlike% "perc|percap|percapita*|per capita|percent" & 
                          !description %rlike% "percap|percapita|per capita|percent") #%>% collect() 
...
sdf_bind_rows(rhs_long_notcounts,
              rhs_long_percap %>% dplyr::select(-population)) %>% 
              spark_write_parquet("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/data_out/rhs_combined/", mode="overwrite")
...
rhs_codebook <- rhs_combined %>% 
                 dplyr::select(variable,  description,  dataset) %>% 
                 sdf_distinct() %>% #don't do regular distinct or it'll take crazy long
                  mutate(category_sex = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "sex|male|women|men|female") %>%
                  mutate(category_age = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "age|years_old|^vap_") %>%
                  mutate(category_race = dataset  %rlike%  "census_2020_actual|census2020|acs5|countyhealthrankings|prri" & variable  %rlike%  "race|black|indian|hispanic|white|ancestry|asian|hawaiian|language|pop_aian|vap_hisp|vap_two|vap_other|vap_aian|pop_two_|pop_nhpi|pop_hisp|pop_other") %>%
                  mutate(category_citizenship = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "citizenship|naturalization") %>%
                  mutate(category_foreign_born = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "nativity|foreign|place_of_birth") %>%
                  mutate(category_marriage = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "marital|spouse|widowed") %>%
                  mutate(category_education = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "education|school|college|degree") %>%
                  mutate(category_wealth_employment = dataset  %rlike%  "census_2020_actual|census2020|acs5|countyhealthrankings" &  variable  %rlike% 
                           "poverty|income|earnings|food_stamps|labor_force|employment|earner|owner_costs") %>%
                  mutate(category_longer_term_mobility = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "geographical_mobility") %>%
                  mutate(category_transportation = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "transportation") %>%
                  mutate(category_occupation = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "work|ocupation|employment") %>%
                  mutate(category_households = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "household|rooms|vacancy|allocation_of|allocation_rate|structure_built|units_in_structure") %>%
                  mutate(category_amenities = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "vehicle|telephone|internet") %>%
                  mutate(category_population = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "population|^pop_") %>%
                  mutate(category_family_structure = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "children|adults|grandchildren|family type|birth") %>%
                  mutate(category_disabilities = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "disability|hearing|vision|cognitive|ambulatory|difficulty|disabilities") %>%
                  mutate(category_insurance = dataset  %rlike%  "census_2020_actual|census2020|acs5|countyhealthrankings" & variable  %rlike%  "insurance|medicaid|va health care|tricare|uninsured") %>%
                  mutate(category_military = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "veteran|civilian|military|tricare") %>%
                  mutate(category_housing = dataset  %rlike%  "census_2020_actual|census2020|acs5" & variable  %rlike%  "housing|occupancy|tenure|mortgage|heating|rent|value|price|mortgage") %>%
                  mutate(category_economy_employment = dataset  %rlike%  "bae|cew") %>%
  
                  mutate(category_health_conditions = dataset  %rlike%  "countyhealthrankings"  & variable  %rlike% "obesity|smoking|birthweight|fair_health|low_birthweight|hospital_stays|mortality|births" ) %>%
  
                  mutate(category_healthcare_services = dataset  %rlike%  "countyhealthrankings"  & variable  %rlike% "dentists|mammography|mental_health|physicians" ) %>%
                  mutate(category_covid = dataset  %rlike%  "covid19projections" ) %>%
                  mutate(category_google_mobility = dataset  %rlike%  "google" & description  %rlike% "mobility"  ) %>%
                  mutate(category_vaccine_supply = dataset  %rlike%  "vaccinedotgov" | variable %rlike% "supply_vaccine|google_vaccine_facilities_count" ) %>%
                  mutate(category_vaccine_demand =  variable %rlike% "countyhealthrankings_flu_vaccinations|percent_accept_vaccination" ) %>%
                  mutate(category_voting = dataset  %rlike%  "MIT Election Data and Science Lab, 2018, County Presidential Election Returns 2000-2020") %>%
                  mutate(category_masking = dataset  %rlike%  "NYT") %>%
                  mutate(category_mobility_safegraph = dataset  %rlike%  "safegraph") %>%
                  mutate(category_religion = dataset  %rlike%  "therda|prri" ) %>%
                  mutate(category_va_vaccinations = variable  %rlike%  "through_va") %>%
                  mutate(category_warning_dv = variable  %rlike%  "WARNING|WARNING_DV_") %>%
                  mutate(category_global_warming = dataset  %rlike%  "yale") %>%
                 collect() %>%
                 ...
                 mutate(variable_clean_dupe=duplicated(variable_clean)) %>% #for these variable names
...
rhs_codebook %>% dplyr::select(starts_with('category')) %>% colSums() %>% sort()
rhs_codebook %>% dplyr::select(starts_with('category')) %>% rowSums() %>% table() #There are 106 with no labels

temp <- rhs_codebook
temp$rowsum <- rhs_codebook %>% dplyr::select(starts_with('category')) %>% rowSums()
temp %>% filter(rowsum==0) %>% View()
...
#This just adds the variable clean and variable clean 255 to the long version as well as makes sure there's no dupes for when go to wide
rhs_combined_long <- rhs_combined %>% dplyr::select(fips, variable,value)  %>%
                      inner_join(rhs_codebook_spark %>% filter(!variable_clean_dupe) %>% dplyr::select(variable, variable_clean, variable_clean_255)) %>% #inner join restricts it to just things in the codebook
                      dplyr::select(-variable) %>% #
                      mutate(fips=as.numeric(fips)) %>%
                      filter(!is.na(fips))  %>% 
                      dplyr::select(fips,variable_clean_255,value) 
...
dim(rhs_combined_long)  #72942165
...
rhs_combined_wide <- rhs_combined_wide_temp %>%
                      pivot_wider(names_from=variable_clean_255, values_from=value) 
                      #sdf_pivot(fips ~ variable_clean, #for whatever reason it is really bad at going long to wide
                      #  fun.aggregate = list(value = "mean")
                      #)
dim(rhs_combined_wide) #24270 #5093 35,583 #sticking with only or nearly only percentages reduces the cols to 35k
...
folds_df <- states_sf_tigris_continental %>% as.data.frame() %>% dplyr::select(fips_state=STATEFP, fold=fold_5_collapsed) %>% mutate(fips_state=as.numeric(fips_state))
...
dim(x_all) #24,268 #4600 152511
x_all %>% colnames() %>% str_detect("acs1") %>% table()
