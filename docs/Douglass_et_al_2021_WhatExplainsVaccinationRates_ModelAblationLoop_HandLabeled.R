...
get_lgbm_cv_preds <- function(cv){
  ...
  for(group in variable_groups){
    ...
    condition <- rhs_codebook_total_coded[,group] %in% 1
    ...
    for(fold in folds){
      shap_out_file=glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results/shap/treeshap_long_ablation{ablation}_fold{fold}.parquet")
