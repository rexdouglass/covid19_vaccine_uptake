...
registerDoParallel(cl)
  ...
  for(group in variable_groups){
    ...
    for(fold in folds){
      ...
      length(vars_pruned)
      dtrain_pruned=lgb.Dataset( x_train[,vars_pruned, drop=F]  %>% Rfast::data.frame.to_matrix(col.names=T) , #it's throwing an error about string size, i'm going to withhold feature names just incase that's the reason
      ...
      scoringFunction <- function(max_depth=-1L,  num_leaves=2, bagging_fraction=1, feature_fraction=1, features_to_keep) { #min_data_in_leaf learning_rate
        ...
        try({
          dtrain_pruned_subset=lgb.Dataset( x_train[,variable_shape_order[1:features_to_keep], drop=F]  %>% Rfast::data.frame.to_matrix(col.names=T) , #it's throwing an error about string size, i'm going to withhold feature names just incase that's the reason
                                     label=yid_train[,'y_share18plus']  %>% Rfast::data.frame.to_matrix(col.names=T), feature_pre_filter=T, max_bin=255) #I can up it to 255 now
      ...
      #     arrow::write_parquet(glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_include/tuning/scoreSummary_ablation{ablation}_fold{fold}.parquet"), version="2.0") #2.0 did not fix it, now trying
      # 
