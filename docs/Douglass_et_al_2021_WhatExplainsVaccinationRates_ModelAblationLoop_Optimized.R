...
for(round in rounds){
  ...
  for(fold in folds){
    ...
    dtrain_pruned=lgb.Dataset( x_train[,vars_pruned]  %>% Rfast::data.frame.to_matrix(col.names=T) , #it's throwing an error about string size, i'm going to withhold feature names just incase that's the reason
                               label=yid_train[,'y_share14plus']  %>% Rfast::data.frame.to_matrix(col.names=T), feature_pre_filter=T, max_bin=255) #I can up it to 255 now
    ...
    scoringFunction <- function(max_depth,  num_leaves, bagging_fraction, feature_fraction) { #min_data_in_leaf learning_rate
      ...
      return(
        list( 
          Score = lightgbm_cv_pruned$best_score*-1 #it maximizes so you have to do negative root mean squared
          , nrounds = lightgbm_cv_pruned$best_iter
        )
      )
    }
    ...
    y_hat_test_pruned_optimized <- predict(lightgbm_pruned_optimized, x_test[,vars_pruned, drop=F] )
    ...
    #Min.       1st Qu.    Median      Mean   3rd Qu.      Max. 
    #0.0000577 0.0228862 0.0470810 0.0566853 0.0787389 0.3361465 
    ...
    library(treeshap)
    unified <- lightgbm.unify(lightgbm_pruned_optimized, x_train[,vars_pruned] %>% as.data.frame()  )
    #head(unified$model)
    treeshap1 <- treeshap(unified,  x_test[,vars_pruned] %>% as.data.frame() , verbose =1)
