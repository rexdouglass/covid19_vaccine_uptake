...
#Shuffle also doesn't seem to be a problem. When I shuffled the LHS I get consistently random performance no matter what set of features. It doesn't make up structure.

#for the record different sees produces RMSE within of each other so algorithmic variation isn't our biggest problem. 0.00079692
masterseeds=1 #setting seed back to just 1, can increase if anyone wants to confirm above.
#runtype="include"
runtype="exclude"
#runtype="include_shuffled"
...
missingness_df <- data.frame(variable=colnames(x_all),not_missingness=not_missingness,nonzero=nonzero, both=both) 
...
#We dont' exclude these now if the num of features is zero we just next the loop
  ...
  for(group in variable_groups){
    ...
    for(fold in folds){
      ...
      if(runtype=="include_shuffled"){
        ...
        shap_out_file=glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_include_shuffled/shap/treeshap_long_ablation_{ablation}_fold{fold}_seed{masterseed}.parquet")
        performance_out = glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_include_shuffled/performance/yid_test_ablation_{ablation}_fold{fold}_seed{masterseed}.parquet")
      }
      ...
      if(runtype=="include"){
        ...
        interactions=F
        if(group != 'category_withold_nothing'){
          condition1 <- rhs_codebook_total_coded[,group] == T #Includes this category
          condition2 <- rhs_codebook_total_coded %>% dplyr::select(starts_with("category_")) %>% dplyr::select(-starts_with(group)) %>% rowSums() == 0 #includes no other
          condition <- condition1 & condition2
          eligible_variables <- rhs_codebook_total_coded$variable_clean_255[condition]
        } else {
          eligible_variables <- rhs_codebook_total_coded$variable_clean_255 #make them all
        ...
        shap_out_file=glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_include/shap/treeshap_long_ablation_{ablation}_fold{fold}_seed{masterseed}.parquet")
        performance_out = glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_include/performance/yid_test_ablation_{ablation}_fold{fold}_seed{masterseed}.parquet")
        shap_interactions_out_root=glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_include/shap_interactions/treeshap_long_ablation_{ablation}_fold{fold}_seed{masterseed}") #define the root dir here
      }
      ...
      if(runtype=="exclude"){
        ...
        interactions=F
        if(group != 'category_withold_nothing'){
          condition <- !rhs_codebook_total_coded[,group] == T #Exclude anything that contains this category
          eligible_variables <- rhs_codebook_total_coded$variable_clean_255[condition]
        } else {
          eligible_variables <- rhs_codebook_total_coded$variable_clean_255 #make them all
        }
        ...
        shap_out_file=glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_exclude/shap/treeshap_long_ablation_{ablation}_fold{fold}_seed{masterseed}.parquet")
        performance_out = glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_exclude/performance/yid_test_ablation_{ablation}_fold{fold}_seed{masterseed}.parquet")
        shap_interactions_out_root=glue("/mnt/8tb_a/rwd_github_private/TrumpSupportVaccinationRates/results_exclude/shap_interactions/treeshap_long_ablation_{ablation}_fold{fold}_seed{masterseed}") #define the root dir here
      }      

      if(length(eligible_variables)==0){
        print(group); print(" Has no unique features to only that group"); next
      }
      ...
      if(shuffle){
        yid_train$y_share18plus <- sample(yid_train$y_share18plus)
        yid_test$y_share18plus <- sample(yid_test$y_share18plus)
      }
      
      library(lightgbm)
      print("Fit all features")
      temp <- x_train
      names(temp) <- paste0("blah",1:ncol(x_train))
      dtrain=lgb.Dataset( temp %>% scale()  %>% Rfast::data.frame.to_matrix(col.names=F) , #it's throwing an error about string size, i'm going to withhold feature names just incase that's the reason
                          label=yid_train[,'y_share18plus']  %>% Rfast::data.frame.to_matrix(col.names=F), feature_pre_filter=T, max_bin=15)  #for gpu it says we can get away with 15 bins, smaller number of bins speeds up cpu too when lots of features
      ...
      clusterExport(cl,c('cv_folds_list','x_train','yid_train','variable_shape_order','scoringFunction','masterparams'))
      ...
      suppressMessages(
        yid_test %>% mutate(y_hat_test_pruned_optimized=y_hat_test_pruned_optimized, fold=fold, ablation=ablation, seed=masterseed) %>% arrow::write_parquet(performance_out)
      )
      ...
      if(interactions){
        ...
        for(var_num in 1:k){
          print(var_num)
          shap_interactions_out=glue("{shap_interactions_out_root}_variable{var_num}.parquet")
          ...
          df_long2 %>% arrow::write_parquet(shap_interactions_out, version="2.0") #2.0 did not fix it, now trying
        }
      }
